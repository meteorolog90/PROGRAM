...
YEARLY = os.path.join(DATA_DIR, 'CARPATGRID_PREC_Y.ser')
MONTHLY = os.path.join(DATA_DIR, 'CARPATGRID_PREC_M.ser')
DAILY = os.path.join(DATA_DIR, 'CARPATGRID_PREC_D.ser')
GRID = os.path.join(DATA_DIR, 'PredtandfilaGrid.dat')

DB_FILE = 'carpatclimPREC.sqlite3'
    ...
    for df_ in pd.read_csv(filename, sep=r'\s+', chunksize=chunksize):
        ...
        with conn:
            for year, month in df_.index:
                ...
                for prec, cell in zip(df_.loc[year, month], df_.columns):
                    columns_to_insert.append([date, cell, prec])
                    rows += 1
                    LOGGER.debug('Columns prepared for DB insert: %s.',
                                 len(columns_to_insert))
    ...
    for df_ in pd.read_csv(filename, sep=r'\s+', chunksize=chunksize):
        ...
        with conn:
            for year, month, day in df_.index:
                ...
                for prec, cell in zip(df_.loc[year, month, day], df_.columns):
                    columns_to_insert.append([date, cell, prec])
                    rows += 1
                    LOGGER.debug('Columns prepared for DB insert: %s.',
                                 len(columns_to_insert))
